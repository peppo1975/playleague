...
<table border="0">
		...
		<? foreach($matches as $match): ?>		
			...
			<tr>
				<td colspan="5" width="185"><?=$match['Causalresult']['Descrizione'];?></td>
...
<?if($export == 'pdf'):?>
<? $fpdf->output('files/pdf/gestione_campionati_notegara_'.Inflector::Slug($nome_campionato).'.pdf', 'F'); ?>
<script type="text/javascript">
location.href = '/files/pdf/gestione_campionati_notegara_<?=Inflector::Slug($nome_campionato);?>.pdf';
</script>
<?else:?>
<?$handle = fopen('files/xls/gestione_campionati_notegara_'.Inflector::Slug($nome_campionato).'.xls',"w+");?>
<?fwrite($handle, iconv('utf8', 'cp1252', $table_xls));?>
<?fclose($handle);?>
<script type="text/javascript">
location.href = '/files/xls/gestione_campionati_notegara_<?=Inflector::Slug($nome_campionato);?>.xls';
</script><?endif;?>