...
<? $fpdf->SetMargins('12','5.0'); ?>
	...
	<? foreach($almanacco as $campionato): ?>
		...
		<? $fpdf->SetY(150); ?>
		<? $fpdf->Image(APP . '/webroot/img/logo_midland_pdf.jpg',60,50,30); ?>
		...
		<? foreach($campionato['Gironi'] as $girone): ?>	
			...
			<? foreach($girone['Squadre'] as $squadra): ?>
			    ...
			    $thumbnail_link = $thumbnail->link(array('path' => $squadra['Upload']['Squadra'][0]['path'], 'w' => 600, 'q' => '60', 'f' => 'jpg'));
			    $info_image = getimagesize(APP . 'webroot' . $thumbnail_link);
				...
				<table width="125" border="1" cellspacing="0" cellpadding="0">					
					<tr>
				...
				<table width="125" border="1">
					...
					<tr>
						<?
							...
							$storia = html_entity_decode($storia, ENT_QUOTES);
							$storia = strip_tags($storia);
							$storia = trim($storia);
						
						?>
						<td width="125" size="9px"><?=$storia;?></td>
					</tr>
				</table>
				...
				<? foreach($squadra['Upload']['Sponsor'] as $sponsor): ?>
				    ...
				    $thumbnail_link_sponsor = $thumbnail->link(array('path' => $sponsor['path'], 'w' => 474, 'q' => '60'));
			    ...
			    $thumbnail_link = $thumbnail->link(array('path' => $squadra['Upload']['SponsorEsterno'][0]['path'], 'w' => 700, 'q' => '60', 'f' => 'jpg'));
			    $info_image = getimagesize(APP . 'webroot' . $thumbnail_link);
