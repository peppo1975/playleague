...
class ThumbnailHelper extends Helper
{
    ...
    function frame_link($options = array())
    {
        ...
        if (!empty($video))
        {
            ...
            if (!empty($video['Upload']['video_preview']) && is_file('/var/www/vhosts/playleaguesport.it/cake-version/app/webroot' . '/' . $video['Upload']['video_preview']))
            {
        ...
        if ($video_preview == null)
        {
            ...
            if (!extension_loaded('ffmpeg'))
                return ("skip ffmpeg extension not loaded");
            if (!extension_loaded('gd'))
                return("skip gd extension not avaliable.");
            if (!is_file($options['src']))
                return ("Video nn trovato");
    ...
    {
        file_put_contents("_image_is_cached", print_r($this,true));
    ...
    public function link($options = array())
    {
        ...
        if (!isset($options['save_path']))
            $options['save_path'] = '/var/www/vhosts/playleaguesport.it/cake-version' . '/app/webroot/assets/images/thumbs';
        //GIUSEPPE 2017-02-13 ... inserito percoro assoluto che si "riconosce automaticamente"
        //if (!isset($options['save_path'])) $options['save_path'] =  $_SERVER['DOCUMENT_ROOT'] . '/assets/images/thumbs';
        if (!isset($options['display_path']))
            $options['display_path'] = '/assets/images/thumbs';
        if (!isset($options['error_image_path']))
            $options['error_image_path'] = '/assets/images/error.jpg';

        if (!isset($tag_options['alt']))
            $tag_options['alt'] = "";
    ...
    public function show($options = array(), $tag_options = array())
    {
        ...
        if (!isset($options['save_path']))
        {
            $options['save_path'] = APP . 'webroot/assets/images/thumbs';
//            $options['save_path'] = '/var/www/vhosts/midlandsport.it/cake.midlandsport.it/midland2015cake2' . '/app/webroot/assets/images/thumbs';
        }
        ...
        if (!isset($options['display_path']))
        {
            $options['display_path'] = APP . 'webroot/assets/images/thumbs';
            //            $options['display_path'] = '/assets/images/thumbs';
        }
        ...
        if (!isset($options['error_image_path']))
        {
            $options['error_image_path'] = APP . 'webroot/assets/images/error.jpg';
            //            $options['error_image_path'] = '/assets/images/error.jpg';
        }
