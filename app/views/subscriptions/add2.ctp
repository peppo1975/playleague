...
<p>
	TESSERAMENTO DI ATLETI E/O DIRIGENTI<br />
	<br />
	Alla manifestazione possono partecipare solo i tesserati per la stagione sportiva in corso.<br />
	Ogni squadra dovr&agrave; avere un minimo consigliato, iniziale di giocatori tesserati, 8 per C5 e 10 per C7, non esiste un limite massimo al numero dei tesserati.<br />
	L&rsquo;accettazione del tesseramento sar&agrave; a discrezione del consiglio L.D.A. Midland.<br />
	Sono soggetti a tesseramento anche gli elementi facenti parte dello staff tecnico, della dirigenza e gli accompagnatori della squadra che intendono seguire la stessa all&rsquo;interno del terreno di gioco.</p>
...
<? if (!$this->Session->read('Login.data') && 1==0): ?>
	...
	$(".resptype").change(function() {
		...
		if (val == 1) {
			...
			$(".respname[data-index=" + index + "]").hide();
				...
				<?=$this->Form->create('Subscription', array('url' => '/subscriptions/add2', 'id' => 'subForm','autocomplete'=>'off'));?>
