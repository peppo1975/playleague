<div class="tab-squadra">
		<div class="list-tab">
			<ul>
				...
				<? endif; ?>
				<li class="selected"><a href="/squadra/dettaglio/<?=$squadra['Squadre']['Squadra'];?>/<?=strtolower(Inflector::Slug($squadra['Squadre']['Denominazione'],'-'));?>/3<? if(isset($_GET['option']) && !empty($_GET['option'])) echo '?option=' . $_GET['option'];?>" title="giocatori / statistiche <?=$squadra['Squadre']['Denominazione'];?>">Giocatori / Statistiche</a></li>
			    ...
			    anno.next('.values-of-select').find('li:eq(0)').click();
								...
								<div id="filter-pad">
									...
									<ul class="switch-button switch-filters hidden">
										...
										<? if ($squadra['Squadre']['Squadra'] == 997): ?>
											<li class="yellow hidden" id="team-button" data-value="squadra2">Squadra/Atleti</li>

										<? else: ?>
																			
										<li class="yellow hidden" id="team-button" data-value="squadra">Squadra/Atleti</li>
										<? endif; ?>
									</ul>
									<!--
