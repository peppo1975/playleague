<?
//GIUSEPPE  2017-03-08 -> filtra la classe
...
//GIUSEPPE  2017-03-08 -> estrae il punteggio atleta tennis

function read_points_athlete_tennis($array_final, $id)
{
    $points = "0";
    foreach ($array_final as $i => $marcatore)
    {
        if ($marcatore[0]['id_atleta_tennis'] == $id)
        {
            $points = $marcatore[0]['goals'];
            break;
        }
    }
    return $points;
...
?>
<? if ($sport == "CALCIO"): ?>
<div class="yearbookSave pull-right btn btn-primary pull-left mb-xl"><a href="javascript:;" title="Salva cambiamenti" style="color: #fff !important;">Salva</a></div>
<? endif; ?>
...
<div id="results-box">
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $('.yearbookSave').one('click', function () {
    ...
    <table id="tableSquadraLogged" class="table-matches table table-condensed table-striped table-bordered">
        ...
        <? foreach ($roster as $k => $atleta): ?>
            ...
            <? elseif ($sport == "TENNIS"): ?>
                ...
                ?>
                    ...
                    <td><?= $atleta['Athlete']['Nome']; ?></td>			
                    <td><?= $atleta['Yearbook']['Tessera']; ?></td>
                    <td align="center" class=""><?= read_points_athlete_tennis($array_final, $atleta['Athlete']['Atleta']); ?></td>			
