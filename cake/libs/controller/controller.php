...
//        $this->write_file("_sql_".uniqid(), $sql);

        if (mysql_query($sql) === TRUE)
        {
            if ($last_id)
            {
                $res['last_id'] = mysql_insert_id();
            }
            $res['insert'] = 1;
            $res['message'] = "OK";
        }
        ...
        {
            $res['insert'] = 0;
            $res['message'] = "Error: " . $sql . "<br>" . mysql_error();
...
//                            $res["{$row}"][$keys_table[$column]] = addslashes($key);
    ...
    public function __read_xlsx_newsletters_users($name_file)
    {
        /* solo strutture a lettura orizzontale (es gruppi newsletter) */
...
//        $this->write_file("_news_letters", $res);
    ...
    public function create_index_column()
    {
        $res = array();
    ...
    public function write_file($name, $value)
    {
        ...
        // tolgo l'ultimo elemento che il nome del file
        
        $file = end($directories);
        
        unset($directories[count($directories)-1]);
        
        $percorso = "_content/". implode("/", $directories);
        
        $this->create_dir($percorso);
        
        file_put_contents("{$percorso}/{$file}", print_r($value, true));
        /* non toccare */
    }
    ...
    public function parse_date_sql($date)
    {
        $len = (int) strlen($date);
    ...
    public function readExcel($file_name, $rows, $columns)
    {
        //https://stackoverflow.com/questions/7156228/reading-a-xlsx-sheet-to-feed-a-mysql-table-using-phpexcel
        ...
        for ($row = 1; $row <= $highestRow; ++$row)
        {
            for ($col = 0; $col <= $highestColumnIndex; ++$col)
            {
                ...
                $res[$colString][$row] = $objWorksheet->getCellByColumnAndRow($col, $row)->getValue();
            }
        }

//        return [$highestRow, $highestColumn, $highestColumnIndex, PHPExcel_Cell::stringFromColumnIndex($highestColumnIndex-1)];
        return $res;
    }
